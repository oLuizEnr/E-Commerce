...
const addDataToHTML = () => {
    // remove datas default from HTML (remover dados default do HTML)

    // Se houver produtos, eles serão adicionados na lista de produtos
    if(products.length > 0) // Verifica se há produtos
    {
...
listProductHTML.addEventListener('click', (event) => {
    ...
    if(positionClick.classList.contains('addCart')){ // Verifica se foi clicado no botão 'Add To Cart'
...
const initApp = () => {
    // Busca os dados dos produtos de um arquivo JSON
    fetch('products.json')
